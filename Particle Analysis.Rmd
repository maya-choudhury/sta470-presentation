...
data2 <- read_excel("CentrifugeData.xlsx")
...
hist(subset_datapred$origfraction)
```
...
```{r}
logreg5 <- glm(origfraction ~ rotation + avgDiameter + particleVolume + particleMass + factor(sample) + expected.vdW, family = quasibinomial(link = "logit"), data = subset_data)
# changed to quasibinomial and used origfraction as response

subset_data$logreg5 <- predict(logreg5, type = "response")
```
...
plot(subset_data$origfraction, subset_data$logreg5)
...
### Visualizations
```{r datavis}
data_for_plot <- subset_data
data_for_plot
ggplot(data=data_for_plot)+geom_violin(aes(x=rotation, y=origfraction, group=rotation))
#comparison plot of everything against each other
#type
```
```{r}
ata_for_plot %>% 
  filter(type=="solosphere") %>% 
ggplot()  + geom_jitter(aes(x=rotation, y=origfraction, color=type), alpha=0.5)
```
```{r}
data_for_plot %>% 
  filter(type!="solosphere") %>% 
...
neg_data %>% 
  ggplot()  + geom_jitter(aes(x=rotation, y=density, color=neg), alpha=0.5)
```
